<script setup>
defineProps({
    paginator: Object // originally it is listing.links
})
        ...
        <div class="flex items-center rounded-md overflow-hidden shadow-lg">
            <div v-for="(link, i) in paginator.links" :key="i">
                <component
                    ...
                    :class="{'hover:bg-slate-300 dark:hover:bg-slate-500' : link.url, 'text-slate-300' : !link.url, 'font-bold bg-indigo-500 dark:text-indigo-400' : link.active}"
                />
            </div>
        </div>
