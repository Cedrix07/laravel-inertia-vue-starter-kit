    ...
    <div v-if="listings">
        <div v-if="Object.keys(listings.data).length">
           <div class="mb-6">
                ...
                <!-- Table -->
                <table class="w-full table-fixed border border-collapse overflow-hidden rounded-md text-sm ring-1 ring-slate-300 dark:ring-slate-600 bg-white shadow-lg">
                    ...
                    <tbody>
                        <tr v-for="listing in listings.data" :key="listing.id"
                            class="border-b border-slate-200 hover:bg-slate-400 dark:bg-slate-800 dark:hover:bg-slate-600 dark:border-slate-600">
                            <td class="w-3/4 p-3 text-left">
                            <div class="flex items-center gap-2">
                                    <img :src="listing.image ? `/storage/${listing.image}` : '/storage/images/listing/default.png'" class="h-10 w-10 rounded-full object-cover object-center" alt="">
                                    <h4 class="font-bold">
                                        {{ listing.title }}
        ...
        <div v-else>
            <p class="text-slate-400 w-full border-b mb-2">No listings yet</p>
        </div>
    </div>
    <div v-else>
